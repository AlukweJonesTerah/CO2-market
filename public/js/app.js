App = {
  ...
  loadContracts: async () => {
    ...
    const greenCreditTokenAddress =
      "0x8dd4E0331a710b5dD5F8245fB1d42301349Cef31";
    ...
    const KYCContractAddress = "0xB466Fa9cDFffC28D757f0fC6c6d7c4fe8a739032";
  ...
  KYCVerification: async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (responseData.ipfsUrl_Document) {
          ...
          console.error("IPFS URL is undefined");
        }
      } else {
        console.error("Failed to submit form", response.statusText);
      }
    } catch (error) {
      console.error("Error during KYC verification:", error.message);
  ...
  FetchKYCIndustry: async () => {
    ...
    for (var j = 0; j <= certificateCount; j++) {
      ...
      const industryWallet = await App.contracts.kyc.methods.industries(j).call()
    ...
    html += `<tr>
        ...
        <td style="color:green;">
          <button onclick="App.verifyKYC('${industryWallet}')" class='btn'>Approve the KYC</button>
        </td>
      </tr>`;
  }
